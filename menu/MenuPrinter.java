...
public class MenuPrinter {
    ...
    private void executeSearchMenu(String promptMessage, SearchStrategy strategy, String reportTitle, String description) {
        Runnable task = () -> {
    ...
    public void printSearchByColorMenu() {
        executeSearchMenu("Введите цвет для поиска: ", new SearchByColor(), "Поиск по цвету ", "Цвет: ");
    }

    public void printSearchByComplectationMenu() {
        executeSearchMenu("Введите комплектацию для поиска: ", new SearchByConfiguration(), "Поиск по комплектации ", "Комплектация: ");
    }

    public void printSearchByYearMenu() {
        executeSearchMenu("Введите год для поиска: ", new SearchByYear(), "Поиск по году ", "Год: ");
    }

    public void printSearchByPowerMenu() {
        executeSearchMenu("Введите мощность для поиска: ", new SearchByPowerRange(), "Поиск по мощности ", "мощность: ");
    }
    ...
    public void printSortByMenu(SortStrategy strategy, String sortType) {
        Runnable task = () -> {
            ...
            sorter.print(cars, "Cортировка по " + sortType + " : ", true);
        };
        threadManager.execute(task);
    }
    ...
    public void printHeader() {
        System.out.println(ANSI_BLUE + "                          ╔═══════════════════" + ANSI_RESET + ANSI_PURPLE +
                " CarMagazine v1.0.1 " + ANSI_RESET + ANSI_BLUE +
                "═══════════════════╗" + ANSI_RESET);
    ...
    private void printThread(int number) {
        ...
        String threadColor = threadManager.isEnabled() ? ANSI_GREEN : ANSI_RESET;

        String descriptionText = "Режим: " + threadStatus;

        System.out.printf("│      %d       │ %-33s │ " + threadColor + "%-57s" + ANSI_RESET + " │\n",
