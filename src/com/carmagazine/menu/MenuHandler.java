...
public class MenuHandler {
    ...
    private boolean handleMainMenuChoice(String choice) {
        switch (choice.trim()) { // <-- убираем пробелы через trim()
            ...
            case "6":
                return handleExportBase();
            case "7":
                return handlePrinterOut();
            case "0":
                return false;
            default:
    ...
    private boolean handleRunTests() {
        return handleQuestChoice(() -> printer.printAllTests());
    }

    private boolean handleExportBase() {
        return handleQuestChoice(() -> printer.printExportBase());
    }

    private boolean handlePrinterOut() {
        return handleQuestChoice(() -> printer.printPrinterOut());
    }
